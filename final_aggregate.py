...
# Function to write the aggregated word counts to a text file
def write_aggregated_word_counts_to_file(global_word_counts, output_file):
    with open(output_file, 'w') as file:
        file.write("Aggregated Word Count:\n")
        for word, count in sorted(global_word_counts.items(), key=lambda x: x[1], reverse=True):
            file.write(f"{word}: {count}\n")
    print(f"\nAggregated word count has been written to {output_file}")


# Main function to split the file, launch workers, and coordinate word counting
def split_file_and_count_words(input_file, output_file, num_processes=None):
    ...
    with Manager() as manager:
        ...
        # Step 6: Write the aggregated word counts to the output file
        write_aggregated_word_counts_to_file(global_word_counts, output_file)
...
if __name__ == '__main__':
    ...
    split_file_and_count_words(input_file, output_file)
